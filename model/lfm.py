...
class Corpus:
    ...
    items_dict_path = 'data/lfm_model/lfm_items.dict'
    ...
    def _get_pos_neg_item(cls, user_id):
        ...
        pos_item_ids = set(cls.frame[cls.frame['user'] == user_id]['item'])
        # 保留item 与 pos_item 并集 - 交集部分
        neg_item_ids = cls.item_ids ^ pos_item_ids
        # neg_item_ids = [(item_id, len(self.frame[self.frame['item'] == item_id]['user'])) for item_id in neg_item_ids]
...
class LFM:
    ...
    def predict(self, user_id, top_n=10):
        ...
        """
        self.load()
        user_item_ids = set(self.frame[self.frame['user'] == user_id]['item'])
    ...
    def save(self):
        ...
        Save model params.
        """
        f = open('data/lfm_model/lfm.model', 'wb')
    ...
    def load(self):
        ...
        Load model params.
        """
        f = open('data/lfm_model/lfm.model', 'rb')
